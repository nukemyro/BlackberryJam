...
bool inNeighbourTable(__u64& neighbour)
{
    for (n = neighbours.begin(); n != neighbours.end(); n++)
        if (n->address == neighbour) { //in neighbour table
...
{ //output Neighbour table
    ...
    int targetIndex = 1;
    for (n = neighbours.begin(); n != neighbours.end(); n++) { //loop for all neighbours in the table
        cout << "\t" << targetIndex << " ";
...
void* Receive(void* dummyPtr)
{
    ...
    while (!quit) {
        ...
        if (frame_control == 0x0080) { //only concerned with beacon frames
            ...
            if (inNeighbourTable(sourceAddress)) { //yes, which one?
                for (n = neighbours.begin();
                     n != neighbours.end(); n++)
                    if (n->address == sourceAddress) { //this one, set arrival true, break
                        n->arrival = true;
                        n->numberOfBeacons++;
                        break;
                    }
            }
            else { //not in neighbour table, add entry
...
void* update(void* dummyPtr)
{
    ...
    while (!quit)
        if (timer.elapsed(UPDATE_INTERVAL)) { //update tables
            ...
            for (n = neighbours.begin();
                 n != neighbours.end(); n++) { /* IMPLEMENTATION OF FIG 2 */
                if (n->arrival) {
                    if (n->lastRecord == -1) {
                        //neighbour found
                    }
                    ...
                    if (n->lastRecord >= MAX_UPDATES) { //neighbour lost!
